...
void usb_device_usio::usio_write(u8 channel, u16 reg, std::vector<u8>& data)
{
	...
	if (channel == 0)
	{
		...
		case 0x2002:
		{
			// Banapassport command (e.g., request to read card or write LED/beep)
			// Just mock log and acknowledge
			usio_log.trace("Banapassport write: cmd 0x%04X, data: %s", reg, fmt::buf_to_hexstring(data.data(), data.size()));
			break;
		}
		default:
		{
...
void usb_device_usio::usio_read(u8 channel, u16 reg, u16 size)
{
	if (channel == 0)
	{
		...
		case 0x2000:
		{
			// Banapassport Reader ID / Status Response
			// Typically returns reader presence + idle status
			response = {0x01, 0x00, 0x00, 0x00}; // example: device present, no card
			usio_log.trace("Banapassport read: status response");
			break;
		}
		case 0x2004:
		{
			// Banapassport Card Poll Response (e.g., UID)
			if (card_inserted)
			{
				response = {0x01, 0x04, 0x12, 0x34, 0x56, 0x78}; // mock card UID
				usio_log.trace("Banapassport read: card inserted, mock UID sent");
			}
			else
			{
				response = {0x00}; // No card inserted
				usio_log.trace("Banapassport read: no card inserted");
			}
			break;
		}
		default:
